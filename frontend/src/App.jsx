...
function App() {
  ...
  const createTodo = async () => {
    ...
    axios.post('http://localhost:3000/', { userId, title })
      .then(response => {
  ...
  useEffect(() => {
    ...
    try {
      const fetchData = async () => {
        setLoading(true)
        const response = await axios.get(`http://localhost:3000/${selectedTodoId}`)
  ...
  const updateTodo = () => {
    ...
    axios.put(`http://localhost:3000/${selectedTodo._id}`, selectedTodo)
      .then(response => {
  ...
  return (
    <div>
      ...
      <ol>
        {todos.map(todo => (
          <li key={todo._id}>
            <div style={{display: 'flex', alignItems: 'center', gap: '1rem'}}><p>{todo.title}</p> - <p>{todo.completed ? 'Completed' : 'Not Completed'}</p> - <button onClick={() => setSelectedTodoId(todo._id)}>Edit</button> <button onClick={() => axios.delete(`http://localhost:3000/${todo._id}`).then(() => axiosFetchTodos())}>Delete</button></div>
          </li>
        ))}
      </ol>
    </div>
